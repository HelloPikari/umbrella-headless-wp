...
if ( ! function_exists( 'pikar_get_post_meta' ) ) {
  /**
   * Post Meta
   *
   ...
   */
  function pikar_get_post_meta( $meta, $compact = false, $hentry = false, $echo = true, $allowed = array() ) {
    ...
    if ( ! $allowed ) {
      ...
      if ( 'post' == get_post_type() ) {
        ...
        $allowed = get_theme_mod( 'post_meta', array());
      }
    }
...
if ( ! function_exists( 'pikar_get_meta_category' ) ) {
  /**
   * Post Сategory
   *
   ...
   */
  function pikar_get_meta_category( $tag = 'span', $compact = false, $post_id = null ) {
    ...
    $output .= '<a href="' . esc_url( get_category_link( $cat->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'pikar_canada' ), $cat->name ) ) . '">' . esc_html( $cat->name ) . '</a>';
...
if ( ! function_exists( 'pikar_get_meta_tags' ) ) {
  /**
   * Post Tags
   *
   * @param string $tag     Element tag, i.e. div or span.
   ...
   * @param int    $post_id Post ID.
   */
  function pikar_get_meta_tags( $tag = 'span', $compact = false, $post_id = null ) {
    ...
    foreach ($post_tags as $post_tag) {
      ...
      $output .= '<a href="' . esc_url( get_tag_link( $post_tag->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'pikar_canada' ), $post_tag->name ) ) . '">' . esc_html( $post_tag->name ) . '</a>' . $separator;
...
if ( ! function_exists( 'pikar_get_meta_date' ) ) {
  /**
   * Post Date
   *
   * @param string $tag     Element tag, i.e. div or span.
   ...
   */
  function pikar_get_meta_date( $tag = 'span', $compact = false, $display = true ) {
    if ( 'post' !== get_post_type() ) return;
    $output = '';
    ...
    $output .= sprintf(
      // __( '<span class="screen-reader-text">Posted on</span> %s', 'pikar_canada' ),
      // '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'

      __( '<span class="sr-only">Posted on</span> %s', 'pikar_canada' ),  $time_string
    );
...
if ( ! function_exists( 'pikar_get_meta_author' ) ) {
  /**
   * Post Author
   *
   * @param string $tag     Element tag, i.e. div or span.
   ...
   * @param bool   $display If post meta is enabled.
   */
  function pikar_get_meta_author( $tag = 'span', $compact = false, $display = true ) {
    if ( 'post' !== get_post_type() ) return;
    $output = '';
    ...
    if ( false === $compact ) {
      $output .= '<span class="meta-separator">' . esc_html__( 'Written by: ', 'pikar_canada' ) . '</span>';
    }
    if ( function_exists( 'coauthors_posts_links' ) ) {
      $output .= coauthors_posts_links( null, null, null, null, false );
        ...
        $output .= sprintf( '<span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s">%3$s</a></span>',
          ...
          esc_attr( sprintf( __( 'View all posts by %s', 'pikar_canada' ), $author_name ) ),
          $author_name
        );
    }
...
if ( ! function_exists( 'pikar_get_meta_reading_time' ) ) {
  /**
   * Post Reading Time
   *
   * @param string $tag     Element tag, i.e. div or span.
   * @param bool   $compact If compact version shall be displayed.
   */
  function pikar_get_meta_reading_time( $tag = 'span', $compact = false ) {

    $reading_time = pikar_get_post_reading_time();
    ...
    if ( true === $compact ) {
      $output .= intval( $reading_time ) . ' ' . esc_html( 'min read', 'pikar_canada' );
    } else {
      /* translators: %s number of minutes */
      $output .= esc_html( sprintf( _n( '%s minute read', '%s minute read', $reading_time, 'pikar_canada' ), $reading_time ) );
    }
...
if ( ! function_exists( 'pikar_calculate_post_reading_time' ) ) {
  /**
   * Calculate Post Reading Time in Minutes
   *
   * @param int $post_id The post ID.
   */
  function pikar_calculate_post_reading_time( $post_id = null ) {
    if ( ! $post_id ) {
      $post_id = get_the_ID();
    }
 ...
 */
function pikar_update_post_reading_time( $post_id, $post, $update ) {
  if ( ! $post_id ) {
    $post_id = get_the_ID();
  }
  $reading_time = pikar_calculate_post_reading_time( $post_id );
  update_post_meta( $post_id, '_pikar_reading_time', $reading_time );
}
 ...
 * @param int $post_id The post ID.
 */
function pikar_get_post_reading_time( $post_id = null ) {
  if ( ! $post_id ) {
    $post_id = get_the_ID();
  }
  // Get existing post meta.
  $reading_time = get_post_meta( $post_id, '_pikar_reading_time', true );
