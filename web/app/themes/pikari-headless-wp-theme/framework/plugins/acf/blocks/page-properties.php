...
function register_page_properties_block() {
  ...
  acf_register_block_type(
    array(
      ...
      'render_callback'   => 'd6_page_properties_block_render_callback',
      'category'          => 'common',
      'icon'              => 'images-alt2',
...
function d6_page_properties_block_render_callback( $block, $content = '', $is_preview = false, $post_id = 0 ) {

    //Only Show in backend
    if ( ! $is_preview ) return;

    //$transparent_nav = get_field('transparent_main_navigation_on_load') ?: 'Author name';
    $transparent_nav = d6_get_field('transparent_main_navigation_on_load');
...
function dev6_register_template() {
    $post_type_object = get_post_type_object( 'page' );
    $post_type_object->template = array(
        array( 'acf/page-properties' ),
    );
}
add_action( 'init', 'dev6_register_template' );
